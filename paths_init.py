...
def write_path(p_raw_files,
    ...
    if ltestsuite:
        p_f_vars_proc = '{}/testsuite/variables_to_process/'\
            .format(os.getcwd())
    else:
        p_f_vars_proc = '{}/variables_to_process/'\
            .format(os.getcwd())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p_raw_files','-pr', dest='p_raw_files',
                        required=True,
                        help='Path to the raw files of the new \
    ...
    parser.add_argument('--p_ref_csv_files',dest='p_ref_csv_files',
                        ...
                        help='Relative or absolute path to \
                            the pool of reference csv-files')

    parser.add_argument('--p_stages',dest='p_stages',
                        default='stages',
                        help='Relative or absolute path to the folder \
                            to store results of the different processing \
                            stages (needs disk space)')

    parser.add_argument('--p_wrkdir', dest='p_wrkdir',
                        default='wrkdir',
                        help='Relative or absolute path to the \
                            working directory (needs disk space)')
