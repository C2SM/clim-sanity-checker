...
def plt_emissions(df_tot, new_exp, df_result, p_out_new_exp = paths.p_stages):
    ...
    filename_mean_std_figures = 'mean_emissions_vars_{}.pdf'.format((new_exp))
    ...
    for ivar,var in enumerate(df_result.variable):
        ...
        act_plt.scatter(xaxis, df_tot_mean[var], c='black',marker='.')

        # plot average reference experiments (grey band)
        m_ref = df_tot[df_tot.exp != new_exp][var]

        act_plt.set_ylim(m_ref.min() * 0.999 ,m_ref.max() * 1.001)

        act_plt.axhline(m_ref.mean(), c = 'k')
        act_plt.fill_between([-1, max(xaxis)-0.5], m_ref.max()*1.0005, m_ref.min()*0.9995, facecolor='grey',alpha=0.6)
        ...
        # title settings
        r_squared = float(df_result[df_result.variable == var]['deviation'])
        act_plt.set_title('{}, deviation = {:.4}'.format(var,r_squared))
...
def plt_pattern_correlation(df_tot, new_exp, df_result, p_out_new_exp = paths.p_stages):
    ...
    filename_mean_std_figures = 'mean_fldcor_vars_{}.pdf'.format((new_exp))
    p_pdf_file_var = os.path.join(p_out_new_exp,filename_mean_std_figures)
    pp = PdfPages(p_pdf_file_var)
    ...
    for ivar,var in enumerate(df_result.variable):
        ...
        # title settings
        r_squared = float(df_result[df_result.variable == var]['R^2'])
        act_plt.set_title('{}, R^2 = {:.4}'.format(var,r_squared))
